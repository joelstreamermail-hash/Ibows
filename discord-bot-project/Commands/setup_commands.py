...
class SetupCommands(app_commands.Group):
    ...
    # ----------------------------------------------------------------------
    # COMMAND: /setup setupbearbeiten (Startet die interaktive UI)
    # ----------------------------------------------------------------------
    @app_commands.command(name="setupbearbeiten", description="Starte die interaktive Konfiguration des Bots.")
    async def interactive_setup(self, interaction: Interaction):
        if not self._is_allowed(interaction):
            return await interaction.response.send_message("❌ Keine Berechtigung.", ephemeral=True)
            
        view = SetupSelectView(interaction, self, is_admin=False)
    ...
    async def handle_edit(self, interaction: Interaction, typ: str, target_id: str):
        await handle_edit_logic(self, interaction, typ, target_id, is_admin_call=False)
        

    @tasks.loop(minutes=4.0)
    async def update_team_panel(self):
        # Logik zum Aktualisieren des Team Panels
        pass 
